"""EPB: Environment Prototype Bank. K prototypes route each sample."""
...
class EPB_TTA:
    def __init__(self,D,C,G,sigma,eps,alpha_s,alpha_f,N_s,N_f,K,device):
        self.D,self.C,self.G,self.device=D,C,G,device;K=min(K,D);B=D//G
        self.B,self.eps,self.K=B,eps,K
        self.env_bank=torch.randn(K,D,device=device)*0.01
        self.slow={'alpha':alpha_s,'N_eff':N_s,'mu':torch.zeros(C,D,device=device),
        ...
        self.fast={'alpha':alpha_f,'N_eff':N_f,'mu':torch.zeros(C,D,device=device),
            'Sigma':[sigma*torch.eye(B,device=device).repeat(C,1,1) for _ in range(G)],
            'Lambda':[torch.inverse(sigma*torch.eye(B,device_device)+eps*torch.eye(B,device_device)) for _ in range(G)],
            'rng':[(i*B,(i+1)*B) for i in range(G)]}
    ...
    def _route_env(self,x):
        ...
        k=dist.argmin().item()
        self.env_bank[k]=(1-self.slow['alpha'])*self.env_bank[k]+self.slow['alpha']*xm
        return k,self.env_bank[k]

    def _fit(self,br,x,y,env):
    ...
    def _upd(self,br):
        for g in range(len(br['rng'])):ov=br['Sigma'][g].mean(0);rg=(1-self.eps)*ov+self.eps*torch.eye(self.B,device=self.device);br['Lambda'][g]=torch.inverse(rg)
    def _pred(self,br,x,env):
        xd=x.float().to(self.device)-env.unsqueeze(0);sc=torch.zeros(1,self.C,device=self.device)
        for g,(l,r) in enumerate(br['rng']):M=br['mu'][:,l:r].T;W=br['Lambda'][g]@M;sc+=xd[:,l:r]@W-0.5*(M*W).sum(0)
        return sc

    def step(self,x,logits,w):
        k,env=self._route_env(x)
        with torch.no_grad():
            ss=self._pred(self.slow,x,env);sf=self._pred(self.fast,x,env)
            ls=logits.cpu()+w*ss.cpu();lf=logits.cpu()+w*sf.cpu()
            ms=ls.softmax(1).max(1)[0].item();mf=lf.softmax(1).max(1)[0].item()
            if ms>mf:wl=ls;wp=ls.softmax(1)
            else:wl=lf;wp=lf.softmax(1)
            self._fit(self.slow,x,wp.to(self.device),env);self._fit(self.fast,x,wp.to(self.device),env)
            self._upd(self.slow);self._upd(self.fast)
        return wl
...
for K in [2,5,10,19]:
    ...
    with torch.no_grad():
        for i in tqdm(range(len(A)),desc=f'K={K}',leave=False):
            ...
            wl=tta.step(x,lgs,w);c+=int(wl.argmax(1).item()==B[i]);w=min(0.01*100/10,0.5)
