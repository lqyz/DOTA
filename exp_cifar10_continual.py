"""BD-TTA Level 2: Continual Stream (CIFAR-10-C, 19 corruptions sequential)."""
...
for ph,(fn,name) in enumerate([(fn,fn.replace('.npy','')) for fn in corrs]):
    ...
    sev5_lbls=labels[5000:10000]
    # Baseline
    c0=0
    with torch.no_grad():
        for i in range(len(data)):
            m(tf(data[i]).unsqueeze(0).to(device));lgs=feats['x']@m.fc.weight.data.T+m.fc.bias.data
            c0+=int(lgs.argmax(1).item()==sev5_lbls[i])
    # TTA
    c=0
    with torch.no_grad():
        for i in tqdm(range(len(data)),desc=f'{ph:2d} {name[:14]}',leave=False):
            m(tf(data[i]).unsqueeze(0).to(device));x=feats['x']
            lgs=x@m.fc.weight.data.T+m.fc.bias.data;wl=tta.step(x,lgs);            c+=int(wl.argmax(1).item()==sev5_lbls[i])
    acc=100*c/len(data);base=100*c0/len(data);results[name]=acc
    print(f'Phase {ph:2d} {name:15s}: base={base:.2f}%  tta={acc:.2f}%')
av=sum(results.values())/len(results)
print(f'\nAVERAGE: {av:.2f}%')
np.save('/root/DOTA/result_continual.npy',{'results':results,'avg':av})
