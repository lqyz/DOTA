"""Unified Independent TTA: alpha=0.002/0.02, N_eff=100, all 19 corruptions."""
...
class DB:
    ...
    def update(s):
        for g in range(s.G):ov=s.S[g].mean(0);rg=(1-s.eps)*ov+s.eps*torch.eye(s.B,device=s.device);s.L[g]=torch.inverse(rg)
    def predict(s,x):
        xd=x.float().to(s.device)-s.mu_env.unsqueeze(0);sc=torch.zeros(1,s.C,device=s.device)
        for g,(l,r) in enumerate(s.rng):M=s.mu[:,l:r].T;W=s.L[g]@M;sc+=xd[:,l:r]@W-0.5*(M*W).sum(0)
        return sc

results={}
for fn in sorted([f for f in os.listdir(path) if f.endswith('.npy') and f!='labels.npy']):
    data=np.load(os.path.join(path,fn))[:50000];m=cifar10_resnet20();m.load_state_dict(sd);m.to(device).eval()
    ...
    with torch.no_grad():
        for i in tqdm(range(50000),desc=fn[:14],leave=False):
            ...
            w=min(0.01*100/10,0.5)
    results[fn.replace('.npy','')]=100*c/50000
    print(f'{fn[:14]}: {results[fn.replace(".npy","")]:.2f}% slow={100*sw/(sw+fw):.0f}%')

av=sum(results.values())/len(results)
print(f'\nAVERAGE: {av:.2f}%')
