"""Continual DA: Severity 5, 19 corruptions, N_eff=100 fixed window."""
...
all_labels=np.load(os.path.join(path,'labels.npy'))
sev5_labels=all_labels[5000:10000]  # Severity 5 labels (5000-9999)
...
# All 19 corruptions, severity 5 (last 5000 images)
...
for phase,(name,fn) in enumerate(seq):
    data=np.load(os.path.join(path,fn))[45000:50000];m=cifar10_resnet20();m.load_state_dict(sd);m.to(device).eval()
    ...
    with torch.no_grad():
        for i in range(N):
            ...
            c+=int(wl.argmax(1).item()==sev5_labels[i])
