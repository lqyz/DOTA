"""Extreme Label Shift: images sorted by class label (class 0→1→...→9), test anti-forgetting."""
...
N=50000
print(f'Loading CIFAR-10-C ({N} per corruption)...')
stream_imgs,stream_lbls,stream_names=[],[],[]
for fn in sorted([f for f in os.listdir(path) if f.endswith('.npy') and f!='labels.npy']):
    data=np.load(os.path.join(path,fn))[:N]
    for i in range(len(data)):
        stream_imgs.append(data[i]);stream_lbls.append(labels[i]);stream_names.append(fn.replace('.npy',''))

print('Sorting by label (class 0→1→...→9)...')
idx=list(range(len(stream_imgs)))
idx.sort(key=lambda i: stream_lbls[i])
stream_imgs=[stream_imgs[i] for i in idx];stream_lbls=[stream_lbls[i] for i in idx];stream_names=[stream_names[i] for i in idx]
print(f'Stream: {len(stream_imgs)} samples')
...
per_class={k:{'c':0,'t':0} for k in range(10)}

with torch.no_grad():
    for i in tqdm(range(len(stream_imgs)),desc='Label Shift'):
        ...
        tl=stream_lbls[i];ok=int(wl.argmax(1).item()==tl);c+=ok;per_class[tl]['c']+=ok;per_class[tl]['t']+=1
...
for cls in range(10):
    a=100*per_class[cls]['c']/per_class[cls]['t'];print(f'  Class {cls}: {a:.2f}% ({per_class[cls]["t"]} samples)')
