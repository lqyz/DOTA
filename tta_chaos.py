"""Fully Shuffled Chaotic Stream: 19 corruptions mixed, 950k samples, long-stream fix."""
...
class DB:
    ...
    def fit(s,x,y):
        ...
        w=y.sum(0);wx=y.T@x
        s.mu=(wx+s.count.unsqueeze(1)*s.mu)/(w.unsqueeze(1)+s.count.unsqueeze(1))
...
idx=list(range(len(stream_imgs)));random.shuffle(idx)
...
cm_s=DB(64,10,4,0.1,0.0001,0.001,device,max_count=200.0)
cm_f=DB(64,10,4,0.1,0.0001,0.005,device,max_count=100.0)
