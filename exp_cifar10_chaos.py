"""BD-TTA Level 3: Chaos Stream (CIFAR-10-C Sev5, 95k shuffled)."""
...
for fn in sorted([f for f in os.listdir(path) if f.endswith('.npy') and f!='labels.npy']):
    ...
    for i in range(5000):A.append(d[i]);B.append(sev5_labels[i])
idx=list(range(len(A)));random.shuffle(idx)
A=[A[i]for i in idx];B=[B[i]for i in idx]
print(f'Chaos stream: {len(A)} samples (Sev5)')
...
with torch.no_grad():
    for i in tqdm(range(len(A)),desc='BD-TTA',leave=False):
        ...
        lgs=x@m.fc.weight.data.T+m.fc.bias.data;wl=tta.step(x,lgs);c+=int(wl.argmax(1).item()==B[i])
